...
INT64 HalDispatchHook(PIMAGE_DATA imageData, PINT64 outStatus) {
	...
	//copy magic first
	if (!SafeCopy(&safeData, imageData, sizeof(safeData.Magic)) || safeData.Magic != 0x6789) {
		return HalDispatchOriginal(imageData, outStatus);
	}

	//copy info struct
	if (!SafeCopy(&safeData, imageData, sizeof(safeData))) {
		return HalDispatchOriginal(imageData, outStatus);
	}
	...
	if (!SafeCopy(&imageData->Buffer, (PVOID)safeData.va, safeData.Length)) {
